...
/*   Updated: 2024/03/13 12:24:40 by martorre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_word(char *str)
{
	...
	while (str[i] != '\0')
	{
		if (ft_isquote(str[i]) == 1 && (isquoteone == 0
				|| isquotetwo == 0) && str[i + 1] != '\0')
			change_val(str, &i, &isquoteone, &isquotetwo);
		if (ft_isquote(str[i]) == 1 && ((isquoteone % 2 == 0)
				|| (isquotetwo % 2 == 0)) && ft_isspace(str[i + 1]) == 1)
			break ;
		if ((ft_isspace(str[i]) == 1 || ft_issign(str[i]) == 1)
			&& ((isquoteone % 2 == 0) && (isquotetwo % 2 == 0)))
			return (i);
		i++;
	}
	i++;
